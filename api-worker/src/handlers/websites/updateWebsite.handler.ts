...
export const updateWebsiteHandler = async (c: Context<{ Bindings: CloudflareEnv }>) => {
  ...
  try {
    ...
    return c.json(WebsiteUpdateFailedErrorSchema.parse({ success: false, message: 'Invalid JSON payload.' }), 400);
  }
  ...
  if (!validationResult.success) {
    ...
    return c.json(WebsiteUpdateFailedErrorSchema.parse({ success: false, message: 'Invalid input for updating website.' }), 400);
  }
  ...
  try {
    ...
    if (updateData.name) {
      ...
      if (existingWebsiteWithName) {
        return c.json(WebsiteNameExistsErrorSchema.parse({ success: false, message: 'Website name already exists.' }), 400);
      }
    }
    ...
    if (setClauses.length === 0) {
      ...
      return c.json(WebsiteUpdateResponseSchema.parse({ success: true, message: 'No effective changes to apply.' }), 200);
    }
    ...
    if (result.success) {
        ...
        return c.json(WebsiteUpdateResponseSchema.parse({ success: true, message: 'Website updated successfully.' }), 200);
    } else {
      console.error('Failed to update website, D1 result:', result);
      return c.json(WebsiteUpdateFailedErrorSchema.parse({ success: false, message: 'Failed to update website.' }), 500);
    }
