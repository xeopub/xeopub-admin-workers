...
export const updateSeriesHandler = async (c: Context<{ Bindings: CloudflareEnv }>) => {
  ...
  try {
    ...
    if (updateData.website_id !== undefined && updateData.website_id !== existingSeries.website_id) {
      const websiteExists = await c.env.DB.prepare('SELECT id FROM websites WHERE id = ?1')
        .bind(updateData.website_id)
        .first<{ id: number }>();
      if (!websiteExists) {
        return c.json(GeneralBadRequestErrorSchema.parse({ success: false, message: 'New website not found.' }), 400);
      }
    }
    ...
    if (updateData.title !== undefined || updateData.website_id !== undefined) {
        const checkTitle = updateData.title !== undefined ? updateData.title : existingSeries.title;
        const checkWebsiteId = updateData.website_id !== undefined ? updateData.website_id : existingSeries.website_id;
        
        if (checkTitle !== existingSeries.title || checkWebsiteId !== existingSeries.website_id) {
            ...
            if (conflictingSeriesByTitle) {
              return c.json(SeriesUpdateFailedErrorSchema.parse({ success: false, message: 'Series title already exists in this website for another series.' }), 400);
            }
        }
    }
    ...
    addField('website_id', updateData.website_id);
    ...
    if (error instanceof Error) {
        ...
        //     return c.json(SeriesUpdateFailedErrorSchema.parse({ success: false, message: 'Series title already exists in this website.' }), 400);
        // }
    }
