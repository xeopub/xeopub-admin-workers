...
export const updatePostHandler = async (c: Context<{ Bindings: CloudflareEnv }>) => {
  ...
  try {
    ...
    if (newSlug && newSlug !== currentPost.slug) {
      let slugCheckQuery;
      const targetWebsiteId = updates.website_id ?? currentPost.website_id;
      const targetSeriesId = updates.series_id !== undefined ? updates.series_id : currentPost.series_id;
    ...
    if (fieldsToUpdate.length === 0) {
      ...
      return c.json(PostUpdateResponseSchema.parse({ success: true, message: 'No changes detected to update.' }), 200);
    }
    ...
    if (result.success && result.meta.changes > 0) {
      return c.json(PostUpdateResponseSchema.parse({ success: true, message: 'Post updated successfully.' }), 200);
      ...
      return c.json(PostUpdateResponseSchema.parse({ success: true, message: 'Post updated successfully (no changes applied).' }), 200);
    } else {
