...
export const deletePostHandler = async (c: Context<{ Bindings: CloudflareEnv }>) => {
  ...
  try {
    ...
    if (result.success && result.meta.changes > 0) {
      return c.json(PostDeleteResponseSchema.parse({ success: true, message: 'Post deleted successfully.' }), 200);
    } else if (result.success && result.meta.changes === 0) {
      // This case should ideally be caught by the check above, but as a fallback:
      return c.json(PostNotFoundErrorSchema.parse({ success: false, message: 'Post not found, nothing to delete.' }), 404);
    } else {
